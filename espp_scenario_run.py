...
class ESPPScenarioRun():
    ...
    def run(self):
        ...
        for period, stock_price in enumerate(self.scenario):
            ...
            if period != 0 and period % self.strategy.company_stock_plan.pay_periods_per_offering == 0 and self.state.ready_to_purchase != 0:
                ...
                shares_purchased_in_period = self.state.ready_to_purchase / stock_purchase_price
                leftover_cash = 0
                cap_hit_irs = False
                ...
                if (self.state.irs_purchase_value + (self.state.last_grant_price * shares_purchased_in_period)) > MAX_PRICE_IRS:
                    ...
                    leftover_cash_irs = self.state.ready_to_purchase - (shares_purchased_in_period_irs * stock_purchase_price)
                    cap_hit_irs = True
                ...
                if (self.state.espp_dollar_value + (stock_purchase_price * shares_purchased_in_period)) > (MAX_PRICE_IRS * self.strategy.company_stock_plan.discount_rate):
                    ...
                    leftover_cash_company = self.state.ready_to_purchase - (shares_purchased_in_period_company * stock_purchase_price)
                ...
                self.state.update_stock_values(shares_purchased_in_period, leftover_cash, stock_price)
        ...
        return (
            self.state.espp_dollar_value + self.state.value_of_money_not_contributed,
            ((1 - self.strategy.capital_gains_tax_rate) * (self.state.espp_dollar_value - self.state.total_contributed)) / self.state.total_contributed
        )
